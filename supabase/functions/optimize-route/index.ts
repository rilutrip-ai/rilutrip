...
async function loadTrustedDayMatrix(
  ...
  const locationFingerprint = await calculateLocationFingerprint(input.activities);
  const { data, error } = await supabaseAdmin
    .from("day_matrices")
...
Deno.serve(async (req) => {
  ...
  const allowed = await checkRateLimit(supabaseAdmin, user.userId, RATE_LIMITS.OPTIMIZE_ROUTE);
  if (!allowed) return jsonResponse({ error: "Too many requests", code: "RATE_LIMITED" }, 429);
